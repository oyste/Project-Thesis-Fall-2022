...
expName = 'arithmetic'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\cowth\\OneDrive - NTNU\\Documents\\FAG2022HØST\\prosjekt\\psychopy ex\\arithmetic.py',
...
instrText = visual.TextStim(win=win, name='instrText',
    text="You will get a series of arithmetic questions.\n\nPress 't' for true and 'f' for false.\n\nPress any key to continue.",
    font='Open Sans',
...
trials = data.TrialHandler(nReps=50.0, method='random', 
    ...
    trialList=data.importConditions('C:/Users/cowth/OneDrive - NTNU/Documents/FAG2022HØST/prosjekt/github/Mental-Health-Fall-2022/dataset collection/psychopy/arithmetic.xlsx'),
...
for thisTrial in trials:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    target.setText(equation)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [target, key_resp]
    ...
    while continueRoutine:
        ...
        if target.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            ...
            mark = str(eq_number) +", "+ corrAns
            print("equation number and answer: [%s]" % mark)    
            outlet.push_sample([mark])  # Push event marker.
        if target.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > target.tStartRefresh + 40-frameTolerance:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['t','f'], waitRelease=False)
